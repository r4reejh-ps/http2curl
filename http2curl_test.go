...
func ExampleGetCurlCommand() {
	...
	command, _ := GetCurlCommand(req, CurlOptions{})
	fmt.Println(command)
...
func ExampleGetCurlCommand_other() {
	...
	command, err := GetCurlCommand(req, CurlOptions{})
	if err != nil {
		panic(err)
	}
	fmt.Println(command)
...
func BenchmarkGetCurlCommand(b *testing.B) {
	...
	for i := 0; i <= b.N; i++ {
		...
		_, err := GetCurlCommand(req, CurlOptions{})
		if err != nil {
			panic(err)
		}
	}
...
func TestGetCurlCommand_serverSide(t *testing.T) {
	svr := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		c, err := GetCurlCommand(r, CurlOptions{})
		if err != nil {
			t.Error(err)
		}
